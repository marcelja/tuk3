...
The simplest way to get the distance of a ride is the usage of spatial points (\textit{ST_POINTS}). This is a data type implemented in SAP Hana, that can be created out of a longitude and a latitude value. Furthermore, there is a function to compute the distance between those points. A single ride starts, when a passenger enters a taxi (occupancy changes from zero to one) and ends, when the passenger leaves (occupancy changes from one to zero). We sum up the lengths between all points sum uo all values for a whole trajectory.
